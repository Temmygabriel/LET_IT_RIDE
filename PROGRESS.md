...
- **← IN PROGRESS: `src/engine.ts`** — the state machine that wires it together: find current window → size bet → place → watch/settle → claim → apply `rules.ts` guardrails → roll. Exposes `step()` (one tick, for the Worker cron) and `run()`. Reading the exact SDK signatures from the installed `.d.ts` first, then push so CI type-checks it.
